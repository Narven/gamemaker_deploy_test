// Checks if the object has an owner set
if (owner != -1)
{
	// Adjusts the coordinates to follow its owner
	x = owner.x;
	y = owner.y;
}

// Checks if a sequence has been set
if (set_sequence != -1)
{
	// Checks if the sequnece has finished
	if (layer_sequence_is_finished(set_sequence) && can_end)
	{
		// Destroys the sequence
		layer_sequence_destroy(set_sequence);
		
		// Sets sequence to -1 as no longer exists
		set_sequence = -1;
		
		// Checks if the sequence has a death call
		if (has_death)
		{
			// Runs the death call
			stored_function();	
		}
		
		// Destroys this sequences manager object
		instance_destroy();
	}
	else
	{
		// Updates the sequence position to the objects
		layer_sequence_x(set_sequence, x);
		layer_sequence_y(set_sequence, y);
	}
}

// Checks if the speed is to be set to the world multiplier
if (is_speed_adjusted)
{
	// Adjusts the speed to correct rate
	layer_sequence_speedscale(set_sequence, (1 / (global.timer_delta * 0.000001)) * global.update_time);	
}

// Checks if sequence is effected by pauses
if (!can_pause)
{
	// Exits out event
	exit;	
}

// Checks if the game is paused
if (global.is_paused || !global.is_window_in_focus)
{
	// Pauses the sequence
	layer_sequence_pause(set_sequence);
}
else
{
	// Checks if the sequence is paused
	if (layer_sequence_is_paused(set_sequence) && !layer_sequence_is_finished(set_sequence))
	{
		// Resumes the sequenece
	    layer_sequence_play(set_sequence);
	}
}